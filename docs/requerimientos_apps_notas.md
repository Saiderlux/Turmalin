...
- Deshacer/rehacer de acciones de ink dentro de la sesión de edición (RF-37)
...
- **RF-37 (deshacer/rehacer de ink)**: El usuario debe poder deshacer y rehacer acciones de ink — trazos añadidos, trazos borrados total o parcialmente — dentro de la sesión de edición de una nota abierta. El historial vive solo en memoria: se reinicia al abrir cada nota, no persiste entre cierres ni reinicios de la app, y nunca se serializa a disco. Acciones sobre links, tags, título y metadata quedan fuera de este sistema — siguen cubiertas por el patrón de notificación no bloqueante existente (RF-34), no se mezclan en el mismo historial. Límite de profundidad: 100 pasos. Al deshacer y luego realizar una acción nueva (no un redo), la pila de rehacer se limpia (comportamiento estándar). En el caso de borrado parcial (RF-05a), deshacer debe restaurar el trazo original completo, no solo los fragmentos sobrevivientes. Los controles de deshacer/rehacer viven en la barra de herramientas, deshabilitados visualmente cuando no hay nada que deshacer/rehacer, sin gestos táctiles que puedan chocar con el pan de un dedo o el zoom de dos dedos (RF-09a/09b).
...
10. **Deshacer/rehacer de ink (RF-37): instantáneas, no deltas.** Cada paso del historial es una instantánea de la lista de trazos de la página tal como estaba ANTES de la mutación. Como `Stroke` e `IdStroke` son inmutables, la instantánea solo copia referencias (costo despreciable con límite de 100 pasos). Deshacer = restaurar la instantánea previa: cubre uniformemente trazo añadido, borrado total y borrado parcial — la instantánea previa contiene el trazo original entero, así que el undo de un recorte parcial restaura el trazo completo, no los fragmentos. Un gesto de goma completo (pen down→up) es UN paso del historial, no uno por evento de movimiento. El historial es por página visitada: cambiar de página lo reinicia (la página que se abandona ya quedó persistida) — el requerimiento solo exige reinicio al abrir la nota; el reinicio por página es una simplificación deliberada del MVP. Deshacer un borrado total que eliminó un link restaura la tinta pero NO el link: el link tiene su propio deshacer (RF-05b) y los historiales no se mezclan por diseño.
11. **Crear link arrastrando en el grafo: descartado.** El layout dirigido por fuerzas (repulsión entre nodos, estilo Obsidian) hace que soltar un nodo exactamente sobre otro sea poco fiable — ni el propio Obsidian lo ofrece. El grafo queda como vista de solo lectura salvo selección (RF-22): la única forma de crear links sigue siendo el Lazo de vínculo (RF-17) desde dentro de la nota.
